...
class Decoder(nn.Module):
    ...
    def forward(self, x_grd, x_arl):   
        ...
        dst = self.decoder(tgt=x_grd, memory=x_arl, 
                           query_pos=query_pos) 
        dst = dst.transpose(1, 2) # 1 x bs x num_queries x dim_embed_dec
        # print("dst: ", dst.size())
...
class MaskDecoder(nn.Module):
    def __init__(
        ...
        transformer_dim = args["dim_embed"]
    ...
    def forward(
        ...
        """
        """
            sparse_prompt_embeddings: bs x dim_embed
            image_embeddings: bs x num_patches2 x dim_embed
        """     
    ...
    def predict_masks(
        ...
        sparse_prompt_embeddings = torch.repeat_interleave(sparse_prompt_embeddings.unsqueeze(1), self.num_queries, dim=1)
        tokens = torch.cat((output_tokens, sparse_prompt_embeddings), dim=1)

        # Expand per-image data in batch direction to be per-mask
        # src = torch.repeat_interleave(image_embeddings, tokens.shape[0], dim=0)
        m = int(image_embeddings.size(1) ** 0.5)
        src = image_embeddings.permute(0, 2, 1).view(image_embeddings.size(0), image_embeddings.size(2), m, m)

        # src = src + dense_prompt_embeddings
        # pos_src = torch.repeat_interleave(image_pe, tokens.shape[0], dim=0)
        pos_src = image_pe 
        b, c, h, w = src.shape
        print("src: ", src.size(), ", pos_src: ", pos_src.size())

		# src, pos_src should be b x emb x h x w
		# tokens should be b x n_query x emb
        # Run the transformer
        hs, src = self.transformer(src, pos_src, tokens)
        print("hs:", hs.size(), ", src:", src.size())
