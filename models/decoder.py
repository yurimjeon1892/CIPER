...
class TwoWayDecoder(nn.Module):
    ...
    def forward(
        ...
        key_img_pe = key_img_pe.flatten(2).permute(0, 2, 1) # 1 x num_patches x dim_embed 
        pos_src = torch.repeat_interleave(key_img_pe, query_img_embedding.shape[0], dim=0) # bs x num_patches x dim_embed 
                
        query_img_embedding = torch.repeat_interleave(query_img_embedding.unsqueeze(1), self.num_queries, dim=1)
