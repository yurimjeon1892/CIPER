...
class Recoder(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.grd_patch_size = (
            int(args["grd_img_size"][0] / args["patch_size"]),
            int(args["grd_img_size"][1] / args["patch_size"]),
        )
        self.arl_patch_size = (
        ...
        self.mlp_mask_grd = MLP(
            args["dim_embed"],
            int(args["dim_embed"] / 4),
            output_dim=args["dim_embed"],
            num_layers=3,
        )
        self.mlp_mask_arl = MLP(
            args["dim_embed"],
            int(args["dim_embed"] / 4),
            output_dim=args["dim_embed"],
            num_layers=3,
    ...
    def get_rng_mask(self, mem_grd_max, mem_arl_max):
        inputs = mem_arl_max.permute((0, 3, 1, 2)).to(
            mem_arl_max.device
        )  # bs x embed_dim x 1 x w
        filters = (
            mem_grd_max.permute((0, 3, 1, 2)).unsqueeze(1).to(mem_arl_max.device)
        )  # bs x 1 x embed_dim x 1 x w'
    ...
    def convert_rng_to_bev_mask(self, rng_mask):
        ...
        bev_mask[
            :,
            :,
            cx - self.center_offset : cx + self.center_offset,
            cy - self.center_offset : cy + self.center_offset,
        ] = 1
        return bev_mask
    ...
    def forward(self, mem_grd, mem_arl):
        ...
        mem_grd = mem_grd.view(
            mem_grd.size(0),
            self.grd_patch_size[0],
            self.grd_patch_size[1],
            mem_grd.size(-1),
        )
        mem_arl = mem_arl.view(
            mem_arl.size(0),
            self.arl_patch_size[0],
            self.arl_patch_size[1],
            mem_arl.size(-1),
        ...
        mem_grd_max, _ = torch.max(
            mem_grd, dim=1, keepdim=True
        )  # bs x 1 x w x dim_embed
        mem_grd_max = self.repeat_elements(
            mem_grd_max, self.repeat_grd_factor
        )  # bs x 1 x grd_feat_width x dim_embed

        mem_arl_max = self.convert_arl_to_rng_feat(
            mem_arl
        )  # bs x 1 x arl_feat_width x dim_embed
