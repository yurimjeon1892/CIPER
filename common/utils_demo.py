...
from common.utils_plot import draw_3dof_pin
...
def add_predicts(args, pred_meta_topk, pred_pose):
    r_earth = 6371000.0
...
def save_output(args, pred_metas, pred_pose, pred_locs, output_dir):
    ...
    top1_pose_img = Image.fromarray(top1_pose_img, "RGB").resize(
    ...
    draw.text(
        (qry_img.size[0] + board_height + 10, 10),
        f"Final result",
        fill="white",
        font=font,
    )
