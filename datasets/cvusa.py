...
class CVUSA(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        if "train" in self.mode:
            ...
            grd_img = Image.open(os.path.join(self.root, self.id_list[idx][1])).convert(
                "RGB"
            )
            arl_img = Image.open(os.path.join(self.root, self.id_list[idx][0])).convert(
                "RGB"
            )
            ...
            arl_img = Image.open(
                os.path.join(self.root, self.id_list[index][0])
            ).convert("RGB")
            img_ref = self.transform_reference(arl_img)
            ...
            grd_img = Image.open(
                os.path.join(self.root, self.id_list[index][1])
            ).convert("RGB")
            img_qry = self.transform_query(grd_img)
