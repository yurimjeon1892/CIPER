...
class KITTI(torch.utils.data.Dataset):
    def __init__(self, mode, args):
        ...
        if "train" in self.mode:
            self.pt_list = args["train_pt_list"]
        elif "valid_cross" in self.mode:
            self.pt_list = args["val_cross_pt_list"]
        elif "valid_same" in self.mode:
            self.pt_list = args["val_same_pt_list"]
        elif "test" in self.mode:
            self.pt_list = args["test_pt_list"]
    ...
    def make_sample_list(self):
        ...
        FileNotFoundError(
            "[i] {} data loaded, size:{}".format(self.mode, len(self.sample_list))
        )

    def __getitem__(self, index):
        if (
            self.mode == "train"
            or self.mode == "valid_same"
            or self.mode == "valid_cross"
        ):
            ...
            left_img_name = os.path.join(
                self.root, "raw", drive_dir, "image_02/data", image_no.lower()
            )
            try:
                grd_img = Image.open(left_img_name, "r")
                grd_img = grd_img.convert("RGB")
            except:
                FileNotFoundError(f"{left_img_name} not exists")
            ...
            arl_rot = arl_img.rotate(-heading / np.pi * 180 + 90.0)
            arl_align_cam = arl_rot.transform(
                arl_rot.size,
                Image.AFFINE,
                (
                    1,
